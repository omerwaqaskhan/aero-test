"""Enhanced Agoda scraper with headless browser support for detailed hotel information."""
...
class AgodaScraper(BaseScraper):
    """Enhanced scraper for Agoda with headless browser support."""
    
    def __init__(self, use_browser: bool = True):
        """Initialize enhanced Agoda scraper."""
        super().__init__(
    ...
    async def __aenter__(self):
        ...
        if self.use_browser:
            try:
                ...
                logger.info("Agoda scraper: Browser launched successfully")
    ...
    async def search_hotels(
        ...
        **kwargs
        ...
        try:
            # Agoda search URL - uses city name (they handle city code lookup internally)
            ...
            # Agoda accepts city name, they'll resolve to city code
            params.append(f"city={quote_plus(destination)}")
            params.append(f"checkIn={check_in.strftime('%Y-%m-%d')}")
            params.append(f"checkOut={check_out.strftime('%Y-%m-%d')}")
            ...
            # Extract hotel listings - try multiple selectors for Agoda
            hotel_elements = []
            ...
            for element in hotel_elements[:50]:
                hotel_info = self._extract_hotel_info(element, destination)
                if hotel_info:
                    hotel_info['source'] = 'agoda'
                    hotels.append(hotel_info)
            
            logger.info(f"Found {len(hotels)} hotels on Agoda for {destination}")
            
        except Exception as e:
            logger.error(f"Error searching Agoda for {destination}: {e}")
        
        return hotels
    
    def _extract_hotel_info(self, element, destination: str) -> Optional[Dict[str, Any]]:
        """Extract hotel information from Agoda element."""
        try:
            ...
            logger.error(f"Error extracting hotel info from Agoda: {e}")
            return None
    
    async def get_hotel_details(self, hotel_url: str) -> Optional[Dict[str, Any]]:
        """Get detailed hotel information from Agoda hotel detail page."""
        if not hotel_url:
            return None
        ...
        try:
            ...
            logger.error(f"Error getting hotel details from Agoda {hotel_url}: {e}")
            return None
