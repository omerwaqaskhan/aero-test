"""Enhanced Hotels.com scraper with headless browser support for detailed hotel information."""
...
class HotelsComScraper(BaseScraper):
    """Enhanced scraper for Hotels.com with headless browser support."""
    
    def __init__(self, use_browser: bool = True):
        """Initialize enhanced Hotels.com scraper."""
        super().__init__(
    ...
    async def __aenter__(self):
        ...
        if self.use_browser:
            try:
                ...
                logger.info("Hotels.com scraper: Browser launched successfully")
    ...
    async def search_hotels(
        ...
        **kwargs
        ...
        try:
            # Hotels.com search URL - correct format
            ...
            # Extract hotel listings - try multiple selectors for Hotels.com
            hotel_elements = []
            ...
            for element in hotel_elements[:50]:
                ...
                if hotel_info:
                    hotel_info['source'] = 'hotels.com'
                    hotels.append(hotel_info)
            
            logger.info(f"Found {len(hotels)} hotels on Hotels.com for {destination}")
            
        except Exception as e:
            logger.error(f"Error searching Hotels.com for {destination}: {e}")
        
        return hotels
    
    def _extract_hotel_info(self, element, destination: str) -> Optional[Dict[str, Any]]:
        """Extract hotel information from Hotels.com element."""
        try:
            ...
            logger.error(f"Error extracting hotel info from Hotels.com: {e}")
            return None
    
    async def get_hotel_details(self, hotel_url: str) -> Optional[Dict[str, Any]]:
        """Get detailed hotel information from Hotels.com hotel detail page."""
        if not hotel_url:
            return None
        ...
        try:
            ...
            logger.error(f"Error getting hotel details from Hotels.com {hotel_url}: {e}")
            return None
